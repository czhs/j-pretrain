...
- If a training or build process is running and no immediate action is required, perform ONE health check, bring durable state fully current (including `state/NEXT_ACTION.md`), write a wait hint (see below), and end the session cleanly. The wrapper sleeps that long, deletes the hint, and relaunches a fresh session.
- The wait hint is a bare integer number of seconds written to `state/WAIT_HINT`, chosen DETERMINISTICALLY from the ETA of the next actionable event and nothing else: ETA ≥ 2 hours → `3600`; ETA between 30 minutes and 2 hours → `900`; ETA under 30 minutes → `300`; ETA unknown → `600`.
- Never infer the wait duration or check-in cadence from ledger timestamps, the spacing of past sessions, or past hint values. That spacing reflects wrapper sleeps, usage-limit gaps, and manual operator restarts — it is operational noise, not precedent or policy.
- When the awaited event is detectable in a file (for example an `EXIT_CODE=` marker appearing in a build or training log), also write `state/WAKE_WHEN`: line 1 the absolute file path, line 2 a literal grep pattern. The wrapper polls this once per minute during the hint sleep and relaunches early when the pattern appears. Update or delete `state/WAKE_WHEN` the moment it no longer applies — a stale, already-true condition would collapse every wait to one minute.
- Do not create or re-arm in-session background watchers, pollers, or notifier tasks for events expected after the session ends: background tasks die at the session boundary, so they can never fire. `state/WAIT_HINT` plus `state/WAKE_WHEN` fully replace them.
